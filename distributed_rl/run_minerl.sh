...
if $leaner; then
	...
	python minerl_learner_node.py -r $redis_server -v $visdom_server -d $actor_device -a $algorithm &
	pids="$pids $!"
    fi
fi
...
if $actor; then
    for i in `seq $1`
    do
	python minerl_actor_node.py -n actor_$i -r $redis_server -v $visdom_server -a $algorithm &
	pids="$pids $!"
    done
fi
