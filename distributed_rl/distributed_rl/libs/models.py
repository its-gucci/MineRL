...
################################################################################
# Create new network with right sizes
#
# The image observations for MineRL are (64, 64, 3) which we reshape to 
# (3, 64, 64). We change the network to reflect that. 
#
################################################################################
class DuelingLSTMDQN2(nn.Module):
    def __init__(self, n_action, batch_size,
                 n_burn_in=40, nstep_return=5,
                 input_shape=(3, 64, 64)):
        super(DuelingLSTMDQN2, self).__init__()
        self.n_action = n_action
        self.batch_size = batch_size
        self.n_burn_in = n_burn_in
        self.nstep_return = nstep_return
    ...
    def to(self, device):
        super(DuelingLSTMDQN2, self).to(device)
        self.hx = self.hx.to(device)
        self.cx = self.cx.to(device)
        return self
...
################################################################################
# Below are the g, f networks for action space representations.
# Adapted from https://arxiv.org/pdf/1902.00183.pdf.
#
# time_skip k is how many actions ahead we predict, since we will map 
# (s_t, s_{t + k + 1}) to and embedding e via g, and e to a_{t:t+k}
# via f
# 
# g is a network that takes in states s, s' and outputs an action representation
# of dimension d_e.
#
# We approximate P(A_t | S_t, S_{t + 1}) = f(a | g(s, s')) = softmax(W_a^T g(s, s'))
#
# f is the network softmax(W_a^T e) where e is the action representation
################################################################################

class GNetwork(nn.Module):

    def __init__(self, embed_dim, time_skip=1, input_shape=3*64*64):
        ...
        self.linear = nn.Linear(32, embed_dim * time_skip)

    def to(self, device):
        super(GNetwork, self).to(device)
        return self
...
class FNetwork(nn.Module):
    ...
    def __init__(self, n_action, embed_dim, time_skip=1):
        super(FNetwork, self).__init__()
        self.embedding = nn.Linear(embed_dim * time_skip, n_action * time_skip)

    def to(self, device):
        super(FNetwork, self).to(device)
        return self
